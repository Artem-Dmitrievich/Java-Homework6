...
// Создание конструктора для создания экземпляров класса
    public laptops(String model, float diagonal, String screenResolution, boolean dvdRom, int hardDisk,
...
// создание метода фильтрации ноутбуков по критериям
    public List<laptops> filter(Set<laptops> laptop) {
        ...
        List<laptops> listLaptop = new ArrayList<>();
        for (laptops tempLaptop : laptop) {
...
// создание метода фильтрации ноутбуков по выборочной критериям
    public Set<laptops> newFilter(Set<laptops> laptop) {
        Scanner scan = new Scanner(System.in);
        Set<laptops> listLaptop = new HashSet<>(laptop);
